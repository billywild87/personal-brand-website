...
export function FloatingParticles() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      <div
        className="absolute top-2 left-1/4 h-1 w-1 animate-pulse rounded-full opacity-40"
        style={{ background: "rgba(34, 211, 238, 0.6)" }}
      />
      <div
        className="absolute top-4 right-1/3 h-0.5 w-0.5 animate-pulse rounded-full opacity-30"
        style={{ background: "rgba(139, 92, 246, 0.6)", animationDelay: "1s" }}
      />
      <div
        className="absolute top-3 right-1/4 h-0.5 w-0.5 animate-pulse rounded-full opacity-20"
